...
@pytest.mark.parametrize("img_path, exp", [
    ...
    (ROOT_DIR + "/tests/imgs/summa.png", "320,00р"),
    ...
    (ROOT_DIR + "/tests/imgs/tinkof_date.png", "05.06.202302:33:24"),  #02:33:24
    (ROOT_DIR + "/tests/imgs/tinkoff.png", "тинькофф"),
    (ROOT_DIR + "/tests/imgs/id_banka_poluch.png", "ИД Банка получателя"),
])
def test_prepared_img(img_path, exp):
    img = cv2.imread((img_path))
    res = _get_results_by_tesseract_configs(img)
    text, conf, r_coef, frame, _ = _get_text_by_max_conf(res[0]) if res[0] else ("", 0, 0, np.ndarray([]))
